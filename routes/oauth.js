...
router.get('/callback', async (req, res) => {
  ...
  if (!locationId && userType === 'Company' && companyId) {
    ...
    try {
      ...
      console.error('Error fetching installed locations:', err.response?.data || err.message);
      return res.status(500).send('Something went wrong resolving your location. Please contact support.');
    }
    ...
    try {
      ...
      console.error('Error minting location token:', err.response?.data || err.message);
      return res.status(500).send('Something went wrong finalizing your install. Please contact support.');
    }

    const expiresAt = new Date(Date.now() + locationTokenData.expires_in * 1000).toISOString();
    ...
    return res.redirect(`${process.env.APP_BASE_URL}/?locationId=${resolvedLocationId}`);
  }
